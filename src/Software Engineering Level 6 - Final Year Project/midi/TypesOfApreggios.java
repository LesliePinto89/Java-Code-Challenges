...
public class TypesOfApreggios {
	...
	public void createApreggio(String name,Chord currentProgChord){
		...
		switch (name){
		case "Clean" : break; 
		case "Adventure" :  adventureApreggio(currentProgChord);
                             playApreggio(200,62); break;
		case "Fine" : fineApreggio(currentProgChord);
		              playApreggio(300,62); break;
		case "Reflective" : curiousApreggio(currentProgChord);
         playApreggio(400,62); break;
         
		case "Exciting" : excitingApreggio(currentProgChord);
        playApreggio(150,0); break;
		}
	...
	public void curiousApreggio (Chord currentProgChord){
		brokenNotes = new ArrayList<Note>();
		int size = currentProgChord.getChordNotes().size()-1;
		Note editNoteEnd = scales.getKey(currentProgChord.getChordNotes().get(size), 5);
			for (Note aNote :currentProgChord.getChordNotes()){
	...
	public void adventureApreggio (Chord currentProgChord){
		...
		int j = 0;
		for (;i<=3;i++){
			//Create list of notes based on progression, and play it sequencentailly with 4 times, each new progression 
			for (Note aNote :currentProgChord.getChordNotes()){
				//for (Note aNote :edited){
			brokenNotes.add(aNote);
